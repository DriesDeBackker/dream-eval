	...
	private final void processUpdate(EventProducerPair update) {
		...
		if (!pairs.isEmpty()) {
			...
			try {
				...
				// log the update time
				updateLog.add(new Update(anyPkt.getSource(), anyPkt.getSourceTime(), System.currentTimeMillis()));
			...
			clientEventForwarder.sendEvent(anyPkt.getId(), event, anyPkt.getSource(), anyPkt.getSourceTime());
